...
const getDepthWithParent = (target, node, parent, depth) => {
  if (node) {
    ...
    const leftAnswer = getDepthWithParent(target, node.left, node.val, depth);
    const rightAnswer = getDepthWithParent(target, node.right, node.val, depth);

    if (leftAnswer.depth) {
      return leftAnswer;
    }

    if(rightAnswer.depth) {
      return rightAnswer;
    }
  }
